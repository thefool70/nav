...
class HermesActionError(RuntimeError):
    """Hermes 已接受 Action，但规划执行失败或未在时限内结束。"""


@dataclass(frozen=True)
class HermesExploreMap:
...
class HermesRestClient:
    ...
    def get_explore_map(self) -> HermesExploreMap:
        """读取并解析当前激光探索栅格图。"""
...
def parse_explore_map(payload: bytes) -> HermesExploreMap:
