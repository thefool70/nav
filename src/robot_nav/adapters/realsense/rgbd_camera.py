...
class RgbdCameraConfig:
    """RealSense 流配置；默认值来自 USB 2 链路实际公布的 profile。"""
...
class RgbdCamera:
    """启动 RealSense，并返回对齐到 RGB 像素坐标的深度图。"""
...
def _validate_config(config: RgbdCameraConfig) -> None:
    """在占用相机前校验流参数。"""
    if not isinstance(config, RgbdCameraConfig):
        raise ValueError("config 必须为 RgbdCameraConfig")
    for name in (
        "color_width",
        "color_height",
        "depth_width",
...
__all__ = ["RgbdCamera", "RgbdCapture", "RgbdCameraConfig"]
