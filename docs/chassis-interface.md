...
Hermes 平移使用 `MotionBlockedError` 表达“新路径受阻事件后，等待仍未恢复有效平移”，
不再只因静止触发 `MotionStalledError`；转向保留原有停滞处理。
该异常须在取消并确认终态后交付，运行层转换为 `PATH_BLOCKED`。
核心仅对 Frontier 探索及恢复暂存方向屏蔽整个区域，其他用途按原有失败分支处理。
事件和底盘时钟读取成为执行依据，失败时取消并停止，不回退到静止猜测。
