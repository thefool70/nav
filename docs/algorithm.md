...
| Frontier | 完整占用图、位姿、首选方向 | 3 m 内已排序的可达候选 | `core/frontier.py` |
...
Frontier 搜索保留底盘完整地图不变，只从机器人所在自由格进行四邻接 BFS，并
扩展到路径距离不超过 3 m 的自由格。候选包括两类：邻接未知区域的可达自由格，
以及位于 3 m BFS 边界且外侧在完整地图中仍为自由格的自由格。后一类按四个
方向合并，供算法以不超过 3 m 的局部段继续探索。
...
Rerun 会把当前一轮 Frontier 直接叠加在占据图中：地图 Frontier 为绿色，3 m
范围 Frontier 为紫色，最终选中的候选为黄色。世界视图仍显示全部历史观测节点
中的候选，因此运行越久，历史候选可能越多；它们不等于当前一轮新生成的候选。
启动时增加 `--debug-frontier`，可在发送移动命令前打印当前候选的栅格坐标、
世界坐标、前沿长度、路径距离、方向奖励和最终分数。
